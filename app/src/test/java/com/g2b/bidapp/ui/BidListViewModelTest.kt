...
class BidListViewModelTest {
    ...
    fun setUp() {
        ...
        every { useCase(any()) } returns flowOf(PagingData.empty())
        viewModel = BidListViewModel(useCase, SavedStateHandle())
    }
    ...
    @Test
    fun `초기 탭은 CNSTWK`() {
        assertEquals(BidCategory.CNSTWK, viewModel.uiState.value.selectedTab)
    }

    @Test
    fun `탭 변경 시 selectedTab 이 업데이트된다`() {
        viewModel.onTabSelected(BidCategory.SERVC)
        assertEquals(BidCategory.SERVC, viewModel.uiState.value.selectedTab)
    }
    ...
    fun `applySearchParams 호출 시 searchParams 가 업데이트된다`() {
        ...
        assertEquals("정보시스템", viewModel.uiState.value.searchParams.keyword)
    }
    ...
    fun `clearSearchParams 호출 시 빈 SearchParams 로 초기화된다`() {
        ...
        assertEquals(SearchParams(), viewModel.uiState.value.searchParams)
    }

    @Test
    fun `isEmpty 가 true 이면 searchParams 가 비어있다`() {
        viewModel.clearSearchParams()
        assertEquals(true, viewModel.uiState.value.searchParams.isEmpty)
    }
